...
try {
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM predefined_items WHERE subcategory_id = ?");
    $stmt->execute([$subcategory_id]);
    $predefined_count = $stmt->fetch(PDO::FETCH_ASSOC)['count'];

    // Check if subcategory is being used in inventory items
    $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM items WHERE subcategory_id = ?");
    $stmt->execute([$subcategory_id]);
    ...
    if ($force_delete && $total_references > 0) {
        // Delete related predefined items first
        if ($predefined_count > 0) {
            $stmt = $pdo->prepare("DELETE FROM predefined_items WHERE subcategory_id = ?");
            $stmt->execute([$subcategory_id]);
        }

        // Update inventory items to remove subcategory reference
        if ($items_count > 0) {
            $stmt = $pdo->prepare("UPDATE items SET subcategory_id = NULL WHERE subcategory_id = ?");
            $stmt->execute([$subcategory_id]);
        }
    }
