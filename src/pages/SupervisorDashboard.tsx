...
const SupervisorDashboard: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 p-2 sm:p-4">
      <div className="w-full max-w-xs sm:max-w-md bg-white rounded-lg relative">
        <div
        ...
        <div className="relative z-10 flex flex-col min-h-screen">
          <Header />
          <main className="flex-1 p-2 sm:p-4 max-w-6xl mx-auto bg-white shadow rounded-lg rounded-tl-none rounded-tr-none flex flex-col">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-xl sm:text-2xl font-bold">Operator Logs</h2>
              <button
                onClick={() => setShowReport(true)}
                className="bg-yellow-600 hover:bg-yellow-700 text-white px-3 py-1 rounded text-xs sm:text-sm"
              >
                Generate Report
              </button>
            </div>
            ...
            {!showReport ? (
              <div className="flex-1 overflow-y-auto max-h-[60vh] rounded border border-gray-300 text-xs sm:text-sm">
                  ...
                  <tbody>
                    {logs.map((log) => (
                      <tr key={log.id} className="border-b hover:bg-gray-100">
                        ...
                        <td className="px-2 py-1 capitalize">{log.action_type}</td>
